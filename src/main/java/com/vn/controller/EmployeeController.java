...
import com.vn.exception.EmployeeNotFoundException;
...
public class EmployeeController {
	...
	public ModelAndView getAllEmployee(Model model) {
		...
		List<Users> e = employeeService.listAll();
	...
	public String createEmployee(@ModelAttribute("employee") @Valid Users employee, BindingResult bindingResult,
		...
		Users username = employeeService.checkUsername(employee.getUsername());
		if (username != null) {
			bindingResult.rejectValue("username", "employee", "Username is already exists !");
		}
		// validate unique email
		Users email = employeeService.checkEmail(employee.getEmail());
		if (email != null) {
		...
		employee.setRole(Const.ROLE_EMPLOYEE);
		employeeService.saveUsers(employee);
	...
	public String getEditUI(@RequestParam("id") Integer id, Model model, RedirectAttributes attributes)
		...
		Users employee = employeeService.get(id);
	...
	public String editEmployee(@RequestParam("id") @Valid Integer id, @ModelAttribute("employee") Users employee,
